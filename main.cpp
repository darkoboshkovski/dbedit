...
static void handleArrowKeyPress(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
    ...
    {
        new_row = E.row;
        new_col = std::min(E.column + 1, E.n_words[new_row] + 1);
    }
...
{
    if (event->keyval == GDK_KEY_Right || event->keyval == GDK_KEY_Left || event->keyval == GDK_KEY_Up ||
...
static gboolean mouseClickCallback(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
    if (event->button != 1)
    {
        return TRUE;
    ...
    unsigned long xRaw = event->x;
    unsigned long yRaw = event->y;

    auto rowColumnData = getRowColumnDataForCoords(xRaw, yRaw);

    auto adjustedRow = std::min(std::min(E.n_rows, E.endRow), rowColumnData.first);
    auto adjustedColumn = std::min(std::min(E.n_words[adjustedRow] + 1, E.endColumn), rowColumnData.second);
    editorMoveCursorToPosition(adjustedRow, adjustedColumn);
...
static void activate(GtkApplication *app, gpointer user_data)
{
    ...
    //    gtk_widget_get_style_context((GtkWidget
    //    *) drawingArea); gtk_style_context_get_background_color(styleContext,
    //    GTK_STATE_FLAG_NORMAL, &backgroundColor);

    gtk_container_add(GTK_CONTAINER(window), (GtkWidget *)drawingArea);

    //    gtk_widget_get_size_request((GtkWidget *) drawingArea, &LAYOUT_WIDTH,
    //    &LAYOUT_HEIGHT);

    //    gtk_window_get_size((GtkWindow *) window, &LAYOUT_WIDTH,
    //    &LAYOUT_HEIGHT);
    ...
    g_signal_new("redraw", G_TYPE_OBJECT, G_SIGNAL_RUN_FIRST, 0, NULL, NULL, g_cclosure_marshal_VOID__POINTER,
                 G_TYPE_NONE, 1, G_TYPE_POINTER);
    g_signal_connect(G_OBJECT(window), "key_press_event", G_CALLBACK(keyPressCallback), (void *)drawingArea);

    g_signal_connect(G_OBJECT(window), "button_press_event", G_CALLBACK(mouseClickCallback), (void *)drawingArea);
